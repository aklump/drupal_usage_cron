 ...
 */
class UsageCron implements UsageCronInterface {
  ...
  public function run() {
    ...
    if (!$this->lock->acquire('usage_cron', 900.0)) {
      // Cron is still running normally.
      $this->logger->warning('Attempting to re-run usage cron while it is already running.');
    }
    else {
      $this->invokeUsageCronHandlers();
      $this->setUsageCronLastTime();

      // Release usage_cron lock.
      $this->lock->release('usage_cron');

      // Return TRUE so other functions can check if it did run successfully.
      $return = TRUE;
    }

    // Process usage_cron queues.
    // TODO Not sure if this is a good idea or should happen 2019-10-24T09:55, aklump.
    //    $this->processQueues();
   ...
   */
  protected function setUsageCronLastTime() {
    // Record cron time.
    $request_time = $this->time->getRequestTime();
    $this->state->set('usage_cron.last', $request_time);
    $this->logger->notice('Usage cron run completed.');
  }
   ...
   */
  protected function invokeUsageCronHandlers() {
    $module_previous = '';
    ...
    foreach ($this->moduleHandler->getImplementations('usage_cron') as $module) {
      ...
      Timer::start('usage_cron_' . $module);

      // Do not let an exception thrown by one module disturb another.
      try {
        $this->moduleHandler->invoke($module, 'usage_cron');
      }
      catch (\Exception $e) {
        watchdog_exception('usage_cron', $e);
      }

      Timer::stop('usage_cron_' . $module);
      $module_previous = $module;
    }
    if ($module_previous) {
      $logger->notice('Execution of @module_previous_cron() took @time.', [
        '@module_previous' => $module_previous,
        '@time' => Timer::read('usage_cron_' . $module_previous) . 'ms',
      ]);
    }
  }
